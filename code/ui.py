...
class Automaton:
    ...
    def transition(self, state, event):
        state_possible = self.TRANSITIONS.get(state, {})
        next_state = state_possible.get(event)
        if next_state is None:
            valid = self.valid_events(state)
    ...
    def call_function(self, state):
        function_name = self.functions.get(state)
        if function_name:
    ...
    def main(self):
        ...
        while self.currentState != State.OUT:
            ...
            valid = self.valid_events(self.currentState)
            ...
            self.currentState = self.transition(self.currentState, action)
            print("Current state :", self.currentState)

            # Calling the function, result of the transition
            self.call_function(self.currentState)
